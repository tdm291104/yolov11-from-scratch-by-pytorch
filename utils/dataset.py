...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_idx = torch.zeros((nl, 1))
        if nl:
            # Ensure cls has the correct shape (n, 1)
            if cls.ndim == 1:
                cls = cls.reshape(-1, 1)
        ...
        return torch.from_numpy(sample), target_cls, target_box, target_idx
    ...
    def collate_fn(batch):
        ...
        # Filter out non-empty tensors and create batch indices
        cls_list = []
        box_list = []
        batch_indices = []
        
        for i, (c, b) in enumerate(zip(cls, box)):
            if c.shape[0] > 0:  # If there are detections in this batch item
                cls_list.append(c)
                box_list.append(b)
                num_detections = c.shape[0]
                batch_idx = torch.full((num_detections, 1), i, dtype=torch.float32)
            ...
            try:
                ...
                if os.path.isfile(b.join(filename.rsplit(a, 1)).rsplit('.', 1)[0] + '.txt'):
                    with open(b.join(filename.rsplit(a, 1)).rsplit('.', 1)[0] + '.txt') as f:
                        ...
                        # Filter out lines that don't have exactly 5 values (class, x, y, w, h)
                        label = [line for line in label if len(line) == 5]
                        if label:
